...
OpenLinkToken uses a multi-rule tokenization strategy to enable privacy-preserving record linkage across datasets that contain PII.
...
OpenLinkToken emits tokens with a `RuleId` of `T1`–`T5`. These identifiers are **rule names**, not “tiers” (they don’t imply an ordering). In practice, different rules tend to trade off precision vs. recall based on which attributes they include.
...
OpenLinkToken normalizes each field before token generation. For full rules, see [Normalization and Validation](normalization-and-validation.md).
...
OpenLinkToken ensures this through:
