...
| `--output`          | `-o`  | Output file path. When omitted the CLI auto-generates a name: `<input_stem>_<subcommand><input_ext>` in the same directory. `package` always defaults to `.zip` (e.g. `data_packaged.zip`). |
