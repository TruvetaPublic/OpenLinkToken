#!/usr/bin/env python3
"""
Hash Calculator for Open Link Token Secrets

This script calculates secure SHA-256 hashes for encryption and hashing secrets
used in the Open Link Token processing flow. The hashes produced by this script
match those generated by the Java application.

Usage:
    python3 hash_calculator.py --hashing-secret "your-hashing-secret" --encryption-key "your-encryption-key"
    python3 hash_calculator.py --hashing-secret "HashingKey" --encryption-key "Secret-Encryption-Key-Goes-Here."

The script outputs the same SHA-256 hashes that are included in the metadata
file generated by the Open Link Token Java application.
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="hash_calculator.py", description="Calculate secure SHA-256 hashes for Open Link Token secrets"
    )

    parser.add_argument("--hashing-secret", help="The hashing secret to calculate hash for", dest="hashing_secret")

    parser.add_argument("--encryption-key", help="The encryption key to calculate hash for", dest="encryption_key")

    parser.add_argument(
        "--output-format", choices=["json", "table", "simple"], default="table", help="Output format (default: table)"
    )
    ...
    if args.output_format == "json":
        ...
        print("Open Link Token Secret Hashes")
        print("=" * 50)
        for key, value in results.items():
            print(f"{key:20}: {value}")
        print("=" * 50)
        print("These hashes match those generated by the Java Open Link Token application")
