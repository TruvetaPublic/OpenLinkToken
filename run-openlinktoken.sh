...
OPTIONAL:
    ...
    --enable-ml1             Enable optional ONNX-backed ML1 token generation (package, tokenize)
    --ml1-model-path PATH    Path to ONNX model used for ML1 inference
    --ml1-tokenizer-path PATH Path to tokenizer.json used for ML1 inference
    --ml1-max-seq-length N   Maximum sequence length used for ML1 tokenization
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --ml1-model-path)
            ...
            ;;
        --ml1-tokenizer-path)
            ML1_TOKENIZER_PATH="$2"
            shift 2
            ;;
        --ml1-max-seq-length)
            ML1_MAX_SEQ_LENGTH="$2"
            shift 2
            ;;
        --help)
            show_usage
...
if [[ $VERBOSE == true ]]; then
    ...
    log_info "ML1 enabled: $ENABLE_ML1"
fi
...
case "$SUBCOMMAND" in
    package)
        ...
        if [[ $ENABLE_ML1 == false ]]; then
            CLI_ARGS+=(--disable-ml1)
        else
            [[ -n "$ML1_MODEL_PATH" ]] && CLI_ARGS+=(--ml1-model-path "$ML1_MODEL_PATH")
            [[ -n "$ML1_TOKENIZER_PATH" ]] && CLI_ARGS+=(--ml1-tokenizer-path "$ML1_TOKENIZER_PATH")
            [[ -n "$ML1_MAX_SEQ_LENGTH" ]] && CLI_ARGS+=(--ml1-max-seq-length "$ML1_MAX_SEQ_LENGTH")
        fi
        ;;
    tokenize)
