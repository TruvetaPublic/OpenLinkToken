...
class PersonAttributesZipWriter(PersonAttributesWriter):
    """Write person attributes as a CSV file inside a ZIP archive."""
    ...
    def close(self) -> None:
        if getattr(self, "_text_stream", None):
            self._text_stream.flush()
            self._text_stream.close()
        if getattr(self, "zip_file", None):
            self.zip_file.close()# SPDX-License-Identifier: MIT
