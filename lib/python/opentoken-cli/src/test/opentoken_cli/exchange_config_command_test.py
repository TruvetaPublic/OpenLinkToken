...
class TestExchangeConfigCommands:
    ...
    def test_encrypt_and_decrypt_round_trip_using_derived_transport_key(self, tmp_path: Path) -> None:
        ...
        tokenize_exit_code = OpenTokenCommand.execute(
            [
                "tokenize",
                "-i",
                str(input_csv),
        ...
        encrypt_exit_code = OpenTokenCommand.execute(
            [
                "encrypt",
                "-i",
                str(tokenized_csv),
        ...
        decrypt_exit_code = OpenTokenCommand.execute(
            [
                "decrypt",
                "-i",
                str(encrypted_csv),
...
def _write_current_exchange_config(exchange_config_path: Path, tmp_path: Path) -> Path:
    ...
    private_key_path = tmp_path / ".opentoken" / "current.private.pem"
    public_key_path = tmp_path / ".opentoken" / "current.public.pem"
