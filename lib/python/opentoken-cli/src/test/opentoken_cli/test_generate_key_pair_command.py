...
class TestGenerateKeyPairCommandIntegration:
    ...
    def test_default_name_uses_iso_date(self, tmp_path):
        """When --name is omitted, files are named opentoken-<YYYY-MM-DD>.*."""
        with patch("pathlib.Path.home", return_value=tmp_path):
            exit_code = OpenTokenCommand.execute(["generate-key-pair"])

        assert exit_code == 0
        opentoken_dir = tmp_path / ".opentoken"
        matches = list(opentoken_dir.glob("opentoken-????-??-??.private.pem"))
        assert matches, "Expected a private key file matching opentoken-<ISO-date>.private.pem"
    ...
    def test_default_curve_is_p256(self, tmp_path):
        ...
        with patch("pathlib.Path.home", return_value=tmp_path):
            exit_code = OpenTokenCommand.execute(["generate-key-pair", "--name", key_name])
    ...
    def test_fails_when_key_already_exists(self, tmp_path):
        ...
        with patch("pathlib.Path.home", return_value=tmp_path):
            first = OpenTokenCommand.execute(["generate-key-pair", "--name", key_name])
            second = OpenTokenCommand.execute(["generate-key-pair", "--name", key_name])
    ...
    def test_directory_created_with_700_permissions(self, tmp_path):
        """~/.opentoken/ is created with 700 permissions."""
        if sys.platform == "win32":
            pytest.skip("POSIX permission test skipped on Windows")
    ...
    def test_private_key_has_600_permissions(self, tmp_path):
        ...
        priv = tmp_path / ".opentoken" / f"{key_name}.private.pem"
    ...
    def test_public_key_has_644_permissions(self, tmp_path):
        ...
        pub = tmp_path / ".opentoken" / f"{key_name}.public.pem"
