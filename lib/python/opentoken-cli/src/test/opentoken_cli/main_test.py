...
class TestOpenTokenCommand:
    ...
    def _create_exchange_config(self, temp_dir: Path, name: str = "test-exchange") -> tuple[Path, Path]:
        """Create an exchange config plus the matching sender private key."""
        ...
        assert exit_code == 0
        private_key_path = temp_dir / ".opentoken" / f"{name}.private.pem"
...
class TestInitiateExchangeViaMain:
    ...
    def test_consumer_help_uses_exchange_config_contract(self, command, capsys):
        ...
        exit_code = OpenTokenCommand.execute([command, "--help"])
        ...
        assert "opentoken-YYYY-MM-DD.exchange.json" in captured.out
