...
class TestTokenizationConfigLoader:
    """Test cases for TokenizationConfigLoader."""

    def test_load_valid_config(self, tmp_path: Path):
        """Loads a valid YAML config into the expected data model."""
...
""".strip(),
    ...
    def test_load_missing_attribute_field_raises(self, tmp_path: Path):
        """Raises a clear error when an attribute mapping omits required 'field'."""
...
""".strip(),
    ...
    def test_load_unknown_field_reference_raises(self, tmp_path: Path):
        """Raises a clear error when a token rule references an unknown field."""
        config_path = tmp_path / "invalid-config.yaml"
        config_path.write_text(
            """
attributes:
